...
Connect balance list to the API and handle loading, empty and error states.

## Test First

Write tests for:

1. Loading state renders before data.
2. Balances render after successful API response.
3. Empty state renders when API returns no balances.
4. Error state renders when API fails.
5. Presentational cards do not fetch data directly.

## Implementation Tasks

1. Create or update `use-balances`.
2. Call `getBalances`.
3. Add loading UI.
4. Add empty UI.
5. Add error UI.
6. Keep `BalanceCard` presentational.

## Acceptance Criteria

- Tests are written first.
- Balance list loads from API.
- User sees clear loading, empty and error states.
- No request submission logic is added here.
