...
Connect manager approval and denial to the mock HCM API.

## Test First

Write integration/component tests for:

1. Pending requests load from API.
2. Balance context is visible.
3. Deny marks request rejected.
4. Deny does not mutate balance.
5. Approve re-checks balance.
6. Approve succeeds when balance is valid.
7. Approve shows conflict when balance changed.
8. Error state is visible and recoverable.

## Product Rule

Manager approval must re-check HCM before confirming.

The UI must not approve based only on stale page data.

## Acceptance Criteria

- Tests are written first.
- Manager flow uses API.
- Approval conflict is handled.
- Denial flow is simple and safe.
