...
Create the recent requests table using local feature data.

## Required Component

```txt
src/app/time-off/components/recent-requests-table/index.tsx
```

## Required Files

```txt
src/app/time-off/enums/request-status.ts
src/app/time-off/types/time-off-request.ts
src/app/time-off/data/requests.ts
```
...
2. Pending request renders.
3. Confirmed request renders.
4. Rejected request renders.
5. Needs Reconciliation request renders.
6. Table columns render.

## Acceptance Criteria

- Tests are written first.
- Table supports all relevant statuses.
- Empty state exists.
