...
Create the static request form with validation.

## Required Component

```txt
src/app/time-off/components/request-form/index.tsx
```

## Required Files

```txt
src/app/time-off/schemas/request-schema.ts
src/app/time-off/types/time-off-request-form-values.ts
```
...
1. All fields render.
2. Submit button renders with `Submit Request`.
3. Required validation works.
4. Days requested must be greater than zero.
5. Submit handler is called with valid values.
